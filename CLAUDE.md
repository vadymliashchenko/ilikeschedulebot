...
`job_start_poll` sends each group's status poll to that choreographer's own private chat with the bot (`db.get_telegram_id_by_choreographer`), not to the shared group — so a choreographer only ever sees polls for their own classes. This requires the choreographer to have already DM'd the bot at least once (a Telegram Bot API restriction: bots can't initiate a private chat). `bot/handlers/poll.py`'s `/start` handler (private chats only) is the self-service onboarding: it shows `keyboards.choreographer_list_keyboard`, and the pick writes to the same `choreographer_links` table used elsewhere for identity matching. A choreographer who hasn't linked yet is silently un-pollable — `job_start_poll` collects these into one warning message to `OWNER_CHAT_ID` instead of failing loudly per-slot. `job_reminder` (who-hasn't-answered) was deliberately *not* changed to private messages — it still posts the shared list to the Team Chat topic.

### Telegram forum topics
The choreographer-facing group is a forum (Telegram "topics") with a "Schedule" topic (polls/table/story images) and a "Team Chat" topic (missed-poll reminders only — chosen deliberately, since choreographers watch that topic more closely). `CHOREO_GROUP_CHAT_ID` and `INSTAGRAM_CHAT_ID` point at the same physical chat id; `SCHEDULE_TOPIC_ID`/`TEAM_CHAT_TOPIC_ID` (optional — `None` for a plain non-forum group) select the topic via `message_thread_id`. `bot/handlers/corrections.py`'s router filters on both `chat.id` and `message_thread_id` together, since chat-id alone doesn't distinguish topics within one forum.
